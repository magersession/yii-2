...
class TransaksiController extends Controller
{
	...
	public function actionPeriksa($id)
	{
		...
		if(isset($_POST['Transaksi']))
		{
			...
			$sql = "SELECT * FROM obat where id_obat = $model->id_obat";
			$query = Yii::app()->db->createCommand($sql)->queryRow();
			$jumlah = $query['jumlah'];
			$hasil = $jumlah - $model->qty;

			// var_dump($result); var_dump($jumlah); var_dump($hasil);die();

			$sql = "UPDATE obat SET jumlah = $hasil where id_obat = $model->id_obat";
			// print_r($sql);die();
			Yii::app()->db->createCommand($sql)->queryRow();

			$result = $query['harga'];
			$total = $result * $model->qty;
			// print_r($total);die();
			$model->total=$total;
